...
INPUT_PATH = Path("data/interim/cleaned/nifty_continuous_futures.parquet")
OUTPUT_PATH = Path("data/processed/features/nifty_futures_features.parquet")
...
def add_rolling_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 1 hour = 4 bars, half day ~ 12 bars, 1 day ~ 25 bars
...
def add_time_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Opening interval flag: first bar of session
    df["opening_interval_flag"] = (df["trade_time"] == "09:30:00").astype(int)
...
def add_gap_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Only meaningful on the first interval of the day
